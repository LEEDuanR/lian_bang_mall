...
@interface LBSilderBarView()<UIGestureRecognizerDelegate>

@property(nonatomic , strong)UIView *coverView;//遮罩
@property(nonatomic ,strong) LBFiltrateViewController *filterView; //筛选视图
...
@implementation LBSilderBarView
...
- (LBFiltrateViewController  *)filterView{
    if (!_filterView) {
        _filterView = [[LBFiltrateViewController alloc]init];
...
#pragma  mark -- show
+(void)lb_showShilderBarView
{
    
   LBSilderBarView * silBar = [[LBSilderBarView alloc]initWithFrame:CGRectMake(0, 0, ScreenW, ScreenH)];
    [KEYWINDOW addSubview:silBar];
